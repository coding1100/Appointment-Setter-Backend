...
from app.api.v1.schemas.auth import (
    UserCreate, UserUpdate, UserLogin, TokenResponse, UserResponse, PasswordChange,
    ForgotPasswordRequest, ResetPasswordRequest, PermissionCreate, RoleCreate,
    LogoutRequest
)
...
async def register_user(
    ...
    try:
        ...
        return UserResponse(
            ...
            updated_at=datetime.fromisoformat(user.get("updated_at", datetime.now(timezone.utc).isoformat()).replace('Z', '+00:00'))
        )
...
async def update_user(
    ...
    try:
        ...
        return UserResponse(
            ...
            created_at=datetime.fromisoformat(updated_user.get("created_at", datetime.now(timezone.utc).isoformat()).replace('Z', '+00:00')),
            updated_at=datetime.fromisoformat(updated_user.get("updated_at", datetime.now(timezone.utc).isoformat()).replace('Z', '+00:00'))
        )
