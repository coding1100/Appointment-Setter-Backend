...
async def create_phone_number(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to create phone number assignment: {str(e)}"
        )
...
async def get_phone_by_agent(agent_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get phone number for agent: {str(e)}"
        )
...
async def assign_phone_to_agent(
    tenant_id: str,
    assignment: AgentPhoneAssignment
):
    """
