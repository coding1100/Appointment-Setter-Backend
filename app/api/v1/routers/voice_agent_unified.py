    ...
    try:
        ...
        if not request.test_mode and not request.phone_number:
            raise HTTPException(
                ...
                detail="phone_number is required when test_mode=false"
            )
...
async def get_tenant_sessions(tenant_id: str, active_only: bool = True):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get tenant sessions: {str(e)}"
        )
...
async def clear_agent_cache(
    ...
    """
    try:
        result = await unified_voice_agent_service.clear_agent_cache(
            tenant_id=tenant_id,
            agent_id=agent_id
        )
        return result
