...
async def reschedule_appointment(
    ...
    try:
        appointment = await appointment_service.reschedule_appointment(
            appointment_id, new_datetime, reason
        )
...
async def complete_appointment(
    ...
    try:
        appointment = await appointment_service.complete_appointment(
            appointment_id, completion_notes
        )
...
async def get_appointments_by_date_range(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get appointments by date range: {str(e)}"
        )
...
async def get_available_slots(
    ...
    try:
        slots = await scheduling_service.get_available_slots_for_date(
            tenant_id, target_date, duration_minutes
        )
...
async def release_slot_hold(hold_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to release slot hold: {str(e)}"
        )