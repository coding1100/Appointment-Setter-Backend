...
from app.api.v1.schemas.tenant import (
    TenantCreate, TenantUpdate, TenantResponse, BusinessInfoCreate, 
    BusinessInfoUpdate, AgentSettingsCreate, AgentSettingsUpdate,
    ServiceTypeCreate
)
...
async def create_business_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create business info: {str(e)}"
        )
...
async def get_business_info(tenant_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get business info: {str(e)}"
        )
...
async def update_business_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to update business info: {str(e)}"
        )

@router.post("/{tenant_id}/agent-settings")
async def create_agent_settings(
    tenant_id: str,
    agent_data: AgentSettingsCreate
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create agent settings: {str(e)}"
        )
