...
class SchedulingService:
    ...
    async def _is_slot_available(
        ...
        for appointment in existing_appointments:
            if appointment.get("status") in ["scheduled", "confirmed"]:
                ...
                if (slot_start - buffer < appointment_end and 
                    slot_end + buffer > appointment_start):
                    return False
    ...
    async def hold_slot(
        ...
        await self.redis_client.set(
            hold_key,
            json.dumps(asdict(hold_data), default=str),
            ttl=hold_duration_minutes * 60  # Convert to seconds
        )
            ...
            for appointment in existing_appointments:
                if appointment.get("status") in ["scheduled", "confirmed"]:
                    ...
                    if (appointment_datetime < existing_end and 
                        appointment_end > existing_start):
