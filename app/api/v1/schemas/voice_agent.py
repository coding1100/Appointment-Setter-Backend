...
class TwilioIntegrationConfig(BaseModel):
    ...
    webhook_url: Optional[str] = Field(None, description="Webhook URL (optional, uses TWILIO_WEBHOOK_BASE_URL if not provided)")
    status_callback_url: Optional[str] = Field(None, description="Status callback URL (optional, uses TWILIO_WEBHOOK_BASE_URL if not provided)")
