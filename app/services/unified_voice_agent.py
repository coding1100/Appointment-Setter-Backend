...
class UnifiedVoiceAgentService:
        ...
        if agent_key not in self.tenant_agents or should_recreate:
            ...
            if voice_id:
                try:
                    ...
                    logger.info(f"✓ SUCCESS: Created ElevenLabs TTS with voice_id={voice_id} (Emily voice), model=eleven_turbo_v2_5, language={language_code}, agent={agent_name}")
            ...
            if metadata and "agent_id" in metadata:
                ...
                try:
                    ...
                    if agent_data:
                        ...
                        logger.info(f"Fetched agent config - voice_id: {voice_id}, name: {agent_data.get('name')}, language: {agent_data.get('language')}")
                    else:
    ...
    async def clear_agent_cache(self, tenant_id: Optional[str] = None, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            "cleared": cleared,
            "message": f"Successfully cleared {cleared} agent instance(s)"
        }
    ...
    async def handle_twilio_webhook(
        ...
        try:
            ...
            if session:
                ...
                logger.info(f"Agent: {agent.get('name')}, Service: {agent.get('service_type')}, Voice: {agent.get('voice_id')}")
    ...
    def _get_system_prompt(self, tenant_config: Dict[str, Any], service_type: str, agent_config: Optional[Dict[str, Any]] = None) -> str:
...
Business: {business_name}"""
    ...
    async def _verify_room_exists(self, room_name: str) -> bool:
        ...
        try:
            room_info = await self.livekit_api.room.list_rooms(
    ...
    def _get_livekit_sip_uri(self, room_name: str) -> str:
        ...
        if not domain or '@' in domain:
            logger.error(f"Invalid SIP domain format: '{domain}'. Should be just the domain (e.g., '3nbu801ppzl.sip.livekit.cloud')")
